...
x = np.save(path + 'fdataX.npy')
y = np.save(path + 'flabels.npy')
...
def baseline_model_saved():
    ...
    json_file = open(path + 'Models/model_2layer_2_2_pool.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    model = model_from_json(loaded_model_json)
    #load weights from h5 file
    model.load_weights(path + "Models/model_2layer_2_2_pool.h5")
    model.compile(optimizer='adam', loss='binary_crossentropy', metrics=[categorical_accuracy])
    return model
...
if(is_model_saved==False ):
    ...
    model_json = model.to_json()
	 ...
	 plt.show()
    with open(path + "Models/model_2layer_2_2_pool.json", "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save_weights("Models/model_2layer_2_2_pool.h5")
    print(path + "Saved model to disk")
else:
...
plt.savefig(path + "Images/Confusion-Matrix_shallow.png")
plt.show()
